...
class ColmapModel:
    def __init__(self, root: str, transform: Callable[[Image, ], Image] = DEFAULT_TRANSFORM):
        self.root = root
        self.transform = transform
        with h5py.File(os.path.join(root, 'metadata.h5'), 'r') as metadata_file:
            self.image_names: list[str] = [name.decode('utf-8') for name in metadata_file['image_name'][()].tolist()]
...
class ColmapDataset(torch.utils.data.ConcatDataset):
    ...
    def __init__(self, root: str, filter: Callable[[str, str], bool] = NO_FILTER, transform: Callable[[Image, ], Image] = DEFAULT_TRANSFORM):
        models = []
...
class ColmapDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str = None):
        if stage == 'fit' or stage is None:
            self.train_dataset = ColmapDataset(self.root, filter=self.train_filter, transform=RANDOM_CROP_TRANSFORM)
            self.val_dataset = ColmapDataset(self.root, filter=self.val_filter, transform=CENTER_CROP_TRANSFORM)
        elif stage == 'test':
            self.train_dataset = ColmapDataset(self.root, filter=self.test_filter, transform=CENTER_CROP_TRANSFORM)
        else:
            raise NotImplementedError(f'Unknown stage {stage}')
    
    def train_dataloader(self):
        return torch.utils.data.DataLoader(self.train_dataset, shuffle=True, **self.loader_kwargs)
    
    def val_dataloader(self):
        return torch.utils.data.DataLoader(self.val_dataset, shuffle=False, **self.loader_kwargs)
    
    def test_dataloader(self):
        return torch.utils.data.DataLoader(self.train_dataset, shuffle=False, **self.loader_kwargs)
