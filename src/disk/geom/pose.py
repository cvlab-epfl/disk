...
class Pose(typing.NamedTuple):
    ...
    def error(p1, p2):
        ...
        θ_deg = RAD_TO_DEG * math.acos(torch.clamp(half_trace, -1.0, 1.0).item()) / 2

        cos_sim = F.cosine_similarity(p1.T.unsqueeze(0), p2.T.unsqueeze(0)).squeeze(0)
        Δ_T = RAD_TO_DEG * math.acos(torch.clamp(cos_sim, -1.0, 1.0).item())
...
def _normalized_cosine_error(v1: ["N"], v2: ["N"]):
    ...
    cos = max(EPS, 1.0 - torch.dot(v1, v2).pow(2).item())
    err = math.acos(math.sqrt(1.0 - cos))
